...
| `rentstar_cookie_consent` | stores your cookie category choices | 1 year           |
| `NEXT_LOCALE`             | remembers your language             | [session/1 year] |

Auth and consent cookies are **strictly necessary**. We currently use **no analytics or
advertising cookies**. If we add analytics later, we will load those scripts only after you
opt in via the cookie banner (see the Cookie Policy). Interface preferences (e.g. theme,
display currency) may be stored in your browser's local storage and never leave your device.
