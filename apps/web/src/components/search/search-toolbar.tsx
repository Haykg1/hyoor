...
export function SearchToolbar({ total, filters }: SearchToolbarProps): React.JSX.Element {
  ...
  function handleSortChange(event: React.ChangeEvent<HTMLSelectElement>): void {
    goToSearch({ ...filters, sortBy: event.target.value as PropertySortValue });
  }
  ...
  return (
    <div className="mb-6 flex flex-wrap items-center gap-3">
      <button
        ...
        disabled
        title="Coming soon"
        className="inline-flex h-8 cursor-not-allowed items-center justify-center gap-2 rounded-md border border-input bg-transparent px-3 text-xs font-medium shadow-sm opacity-60"
      >
