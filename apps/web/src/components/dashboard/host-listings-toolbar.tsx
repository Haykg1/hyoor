...
import type { HostListingStatusFilter } from '@/lib/api/properties';

const ACTIVE_TAB_STATUSES: HostListingStatusFilter[] = ['DRAFT', 'PENDING_REVIEW', 'ACTIVE'];

interface HostListingsToolbarProps {
  searchQuery: string;
  statusFilter: HostListingStatusFilter | null;
  propertyTypeFilter: PropertyType | null;
  showStatusFilter: boolean;
  onSearchChange: (value: string) => void;
  onStatusChange: (status: HostListingStatusFilter | null) => void;
...
export function HostListingsToolbar({
  ...
  onReset,
}: HostListingsToolbarProps): React.JSX.Element {
    ...
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      {showStatusFilter && (
        <select
          value={statusFilter ?? ''}
          onChange={(e) =>
            onStatusChange((e.target.value || null) as HostListingStatusFilter | null)
          }
          className={SELECT_CLASS}
          aria-label={t('status')}
        >
