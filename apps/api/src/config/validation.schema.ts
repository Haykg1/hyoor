import * as Joi from 'joi';

export const validationSchema = Joi.object({
  NODE_ENV: Joi.string().valid('development', 'production', 'test').default('development'),
  PORT: Joi.number().port().default(3001),
  DATABASE_URL: Joi.string().required(),
  JWT_SECRET: Joi.string().min(32).required(),
  JWT_EXPIRES_IN: Joi.string().required(),
  JWT_REFRESH_SECRET: Joi.string().min(32).required(),
  JWT_REFRESH_EXPIRES_IN: Joi.string().required(),
  AWS_REGION: Joi.string().required(),
  // AWS_ACCESS_KEY_ID: Joi.string().required(),
  // AWS_SECRET_ACCESS_KEY: Joi.string().required(),
  // AWS_S3_BUCKET: Joi.string().required(),
  FRONTEND_URL: Joi.string().uri().required(),
  MAIL_FROM: Joi.string().optional(),
  RESEND_API_KEY: Joi.string().allow('').optional(),
  CONTACT_SUPPORT_EMAIL: Joi.string().email().optional(),
  CONTACT_INFO_EMAIL: Joi.string().email().optional(),
  // GOOGLE_CLIENT_ID: Joi.string().required(),
  // GOOGLE_CLIENT_SECRET: Joi.string().required(),
  GOOGLE_CALLBACK_URL: Joi.string().uri().optional(),
  APPLE_CLIENT_ID: Joi.string().allow('').optional(),
  APPLE_TEAM_ID: Joi.string().allow('').optional(),
  APPLE_KEY_ID: Joi.string().allow('').optional(),
  APPLE_PRIVATE_KEY: Joi.string().allow('').optional(),
  THROTTLE_TTL_MS: Joi.number().integer().min(1000).optional(),
  THROTTLE_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_AUTH_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_WRITE_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_PAYMENTS_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_MESSAGING_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_GEOCODING_LIMIT: Joi.number().integer().min(1).optional(),
  THROTTLE_AI_SEARCH_LIMIT: Joi.number().integer().min(1).optional(),
  OPENAI_API_KEY: Joi.string().allow('').optional(),
  OPENAI_MODEL: Joi.string().optional(),
  FIRECRAWL_API_KEY: Joi.string().allow('').optional(),
  PLATFORM_FEE_PERCENT_DEFAULT: Joi.number().min(0).max(100).optional(),
  PAYMENT_LOCK_MINUTES: Joi.number().integer().min(1).optional(),
  AI_SEARCH_GUEST_LIMIT: Joi.number().integer().min(1).optional(),
  AI_SEARCH_VERIFIED_USER_LIMIT: Joi.number().integer().min(1).optional(),
  AI_SEARCH_GUEST_TTL_SECONDS: Joi.number().integer().min(60).optional(),
  TRIP_PLANNER_DAILY_PLAN_LIMIT: Joi.number().integer().min(1).optional(),
  TRIP_PLANNER_JOB_TTL_SECONDS: Joi.number().integer().min(60).optional(),
  TRIP_PLANNER_MAX_COMPLETION_TOKENS: Joi.number().integer().min(500).optional(),
  TRIP_PLANNER_MODEL: Joi.string().optional(),
  TRIP_PLANNER_CANDIDATE_LIMIT: Joi.number().integer().min(1).optional(),
  TRIP_PLANNER_CANDIDATE_CACHE_TTL: Joi.number().integer().min(0).optional(),
  TRIP_PLANNER_MIN_CATALOG_POIS: Joi.number().integer().min(1).optional(),
  TRIP_PLANNER_USE_TEMPLATES: Joi.string().valid('true', 'false').optional(),
  POI_PHOTO_CDN_BASE_URL: Joi.string().allow('').optional(),
  YANDEX_MAPS_API_KEY: Joi.string().allow('').optional(),
  REDIS_URL: Joi.string().allow('').optional(),
  CURRENCY_RATES_API_URL: Joi.string().allow('').optional(),
  CURRENCY_RATES_FETCH_ON_BOOT: Joi.string().valid('true', 'false').optional(),
  POI_SEED_ON_BOOT: Joi.string().valid('true', 'false').optional(),
  TRUST_PROXY: Joi.string().valid('true', 'false').optional(),
  JSON_BODY_LIMIT: Joi.string().optional(),
  MAX_UPLOAD_BYTES: Joi.number().integer().min(1).optional(),
});
